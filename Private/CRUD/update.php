...
function update_book(int $book_id, string $title, int $year, string $publisher, int $genre_id, int $times_borrowed, bool $is_available, array $authors)
{
    ...
    $sql = "UPDATE books SET Title='$title', Publisher='$publisher', PublishYear=$year, GenreId=$genre_id, TimesBorrowed=$times_borrowed, IsAvailable=$is_available WHERE BookId=$book_id";
    $result = mysqli_query($dbConn, $sql);
    if (!$result) {
